...
versão, de que ela foi (\emph{hērpásthē}) raptada de lá e não daqui\footnote{Segundo Acusilao (DK, 9, B, 35) o rapto de Orítia por
...
que dizes, sem nenhum artifício (\emph{atéchnôs})\footnote{Essa \emph{atechnía} ou \emph{ausência de
...
que pode acontecer conosco\footnote{Nessa recitação do discurso de
  ...
  extrair o máximo de prazer e evitar ao máximo os sofrimentos.}.~[231a] Espero que não me advenha nenhum
infortúnio (\emph{atychêsai}) só porque me ocorreu (\emph{tygchánō}) de
não estar te amando (\emph{ouk}~\emph{erastḕs})\footnote{A palavra \emph{erastès},
 ...
 (\emph{epithymías paúsōntai})\footnote{Expressão retomada em
...
quem se livraria (\emph{apotrépein})\footnote{O termo \emph{apotrepein} (231d1) ``livrar,
...
(\emph{phronoûsin}) mal, mas não podem dominar-se (\emph{krateîn})\footnote{Aqui vemos o retrato inicial de Eros
...
(\emph{en tois polloîs})\footnote{Esse
...
o efeito do amor (\emph{mḕ}~\emph{erôntas})\footnote{Optamos pela paráfrase ``aqueles não estão sob
...
opinião (\emph{anti tês dóxês}) dos homens\footnote{Nesse discurso de Lísias ``o melhor'' é
...
para consumar o seu desejo (\emph{epithymías})\footnote{É possível perceber um predomínio, a
...
(\emph{epithymías paúsôntai})\footnote{Período idêntico em 231a. Nesse
  ...
  impulso amoroso para evitar seus efeitos colaterais.}.~[233a] Para os que não estão amando
...
mim do que por um amante (\emph{erastêi})\footnote{Retomada da tese central desse discurso. O
...
prazeroso (\emph{mḕ~hedonês})\footnote{O trecho é desenhado através de uma coordenação
...
pauómenoi tês~epithymías})\footnote{Encontramos novamente a expressão ``cessar do
...
(\emph{hoì pausaménou tês hṓras tόte tḕn hautõn aretḕn epideíxontai}).
...
ocorra o proveito (\emph{ôphéleían}) a ambos\footnote{Essa seria a síntese do discurso,
...
encheram (\emph{peplêrôsthai}) como a uma vasilha\footnote{Esse
  ...
  quem fala, posto que é impulsionada por força alheia.}.~[235d] E foi por
...
(\emph{heúresin})\footnote{São aspectos
...
pleíonos áxia})\footnote{Expressão idêntica à de 235b4-5}~que o de Lísias, estarás em ouro maciço junto a oferenda
dos Cipsélidas\footnote{Cípselo
...
entanto, fica enternecido''\footnote{Menção não literal de 228c1-2, quando Sócrates
...
``vê se entende o que te digo''\footnote{Verso de Píndaro (frag. 105.1 Snell-Maehler) repetido
...
algum deus daqui? -- [236e] e até mesmo a esse plátano\footnote{Esse
...
S: Que infâmia, ó abominável, bem encontraste (\emph{eũ anēũres})\footnote{Continuidade do uso do campo semântico de
...
mḕ~erônti prò toû erôntos déoi charídzesthai})\footnote{Retomada da tese
...
denominamos desmesura (\emph{hýbris})\footnote{Sócrates divide a ação humana em
  ...
  na palinódia (243e-257b6).}.~[238a] A desmesura
...
[agrada] ao não afetado pelo amor\footnote{Essa retomada do
  ...
  está diretamente ligada ao vitupério e ao elogio.}.~[238e] Aos que começam a ser
...
ditado, ``cada idade agrada aos da mesma idade''\footnote{O mesmo provérbio encontra-se
...
os cordeiros''\footnote{Segundo Hermias (§90.9-10 Couvreur = 61,12-13
...
que te ofereci sofra (\emph{páschein}) o que for\footnote{Suspeita, já
  ...
  tal narrativa ímpia.}.~[242a] Quanto a
...
de arte (\emph{atechnôs})\footnote{A palavra \emph{atechnôs} pode ser
  ...
  Fedro, na visão de Sócrates, não detém nenhuma arte.}.~[242b] Por conta dos discursos que
...
Símias de Tebas\footnote{Cf. \emph{Fédon} 59c.}, que é o mais forte (\emph{krateîs}) entre todos. E~agora tu me parece ser a causa (\emph{aitiós}) mesma de outro discurso
...
que ela não altere minha honra (\emph{timàn}) junto aos homens\footnote{Cf. Campbell frg. 310, verso de Íbico também legado por
...
disso, pretenderam-se bondosos e muito civilizados (\emph{asteía})\footnote{Cf. nota 19 acerca de \emph{asteía}.}, mas
...
ocorrido, por vergonha, com a cabeça velada (\emph{egkekalumménos})\footnote{Cf. nota 72, referente 237a4, momento
...
(\emph{epithymô potímôi lógôi oîon halmuràn akoḕn apoklúsasthai})\footnote{O \emph{lógos} tem o poder catártico e de
...
pelo amor (\emph{mè erônti})\footnote{Resgate e inversão da tese central da
...
(\emph{ek theoû}) e a outra junto aos homens (\emph{par'anthópôn})\footnote{Cf. oposição
...
sóbrios (\emph{sôphronoûntos})\footnote{Conservei nesta aparição de
...
sido expostas ao agrado do deus\footnote{A imagem é plasmada (\emph{pláttomen}) da
...
deuses (\emph{éxô theíou choroû})\footnote{Coro dos deuses, sendo que o verbo
...
que a alma se dispõe\footnote{Cf. Plotino, \emph{Acerca do Belo} 7.31.}; as almas dos imortais, quando chegam ao extremo,
...
pálin}) para o interior do céu e chegam à casa\footnote{Héstia, única
...
servem-se do alimento da opinião (\emph{trophêi doxastêi chrôntai})\footnote{Como de costume na
...
valor servem-se corretamente da recordação (\emph{hypomnḗmasin})\footnote{Servir-se das recordações
...
encontram (\emph{heuróntes}) e tudo fazem para que permaneça assim\footnote{Reitera-se nessa passagem que o filósofo é a categoria
...
esforço em olhar para o deus (\emph{pròs tòn theòn blépein})\footnote{Mais uma vez
  ...
  similitudes entre o deus e o amado, uma habilidade dialética.}.~[253a]
...
partilhar (\emph{matascheîn}) da divindade\footnote{A visão é uma
...
ninguém frente ao amigo entusiasmado (\emph{éntheon})\footnote{Hermias
...
(\emph{ophthalmías})\footnote{A oftalmia aqui reaviva a tópica
...
échôn})\footnote{\emph{Eidôlon}
  ...
  1,30,1, Leipzig: Teubner, 1967).}.~[255e] A este nomeia e considera não como amor
...
maioria (\emph{tôn pollôn}) toma por excelente e assim praticam\footnote{A multidão insensata é agora explicitamente
...
redor e debaixo da terra, num percurso sem intelecto (\emph{ánoun})\footnote{Aqui a velha tese de Lísias, repetida por
...
(\emph{ethelêsêi}) contra ele competir (\emph{antiparateînai})\footnote{Fedro reconhece somente aqui a
...
escrever (\emph{gráphein})\footnote{Nesse ponto o diálogo ganha
...
(\emph{epainôsin}), previamente e em qualquer situação\footnote{Os que elogiam e os elogiados nos discursos
...
realizadores\footnote{Hermias ao comentar o trecho, associa Calíope e
  ...
  §180.10-21 Couvreur).}.[259d] Érato (Amorosa) com a [poesia] erótica
...
homens\footnote{Deixei o nome grego praticamente
...
(\emph{tèn homoiótêta tôn óntôn kaì anomoiótêta akribôs dieidénai})\footnote{A arte de enganar, nesse retrato, tem êxito ao
...
\redondo{[…]''.\footnote{Citação de 230e-231a, início da declamação de Lísias. Aqui é a
...
(\emph{to}~\emph{plêthos}~\emph{planâsthai}) e onde não\footnote{Uma definição provisória de arte
...
causa dos nossos maiores bens (\emph{agathôn})\footnote{Aqui vemos uma apresentação da dialética
...
de discursos (\emph{logodaídalon})\footnote{O epíteto de \emph{logodaídalon}, nesse
...
(\emph{psychagôgía}), [271d] aquele que pretende ser rétor
