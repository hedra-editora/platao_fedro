...
\newcommand{\bekker}[1]{{\footnotesize #1}}
...
\bekker{[227a]} \versal{Sócrates}: Ó querido Fedro, de onde vens e para onde vais?

 

\versal{Fedro}: Venho de junto de Lísias, o filho de Céfalo, ó Sócrates. Atravessei o
...
Acúmeno\endnote{Médico, pai de Erixímaco, o qual faz um dos discursos
...
pórtico.\endnote{Dois caminhos, um curto e um longo, servem de metáfora
...
\bekker{[227b]} S: É belo o que dizes, companheiro. E~parece que Lísias estava
na cidade.

F: Sim, com Epícrates\endnote{Epícrates sofreu processo público, para o
  ...
  vol. 1, Kock, \versal{T.} Leipzig: Teubner, 1880). Ainda na \emph{Scholia
  ...
  de bolsa/escudo {[}...{]}''.}, na casa de Morico\endnote{Moriquia ou
...
um banquete (\emph{eistía}) de discursos!\endnote{Cf. Yunis, p.86-87,
...
elevado''\endnote{Píndaro, \emph{Ístmicas} 1,2 (Race, 1997, p.134).}
...
amante (\emph{erônti})\endnote{Primeira enunciação da tese central do
...
comigo e a muitos de nós acontecem.\endnote{Sócrates evidencia, com
  ...
  (\emph{téchnên alupías}, \versal{DK}, 87, \versal{A6}) por meio das palavras, fármacos
  encantadores, tal qual Górgias apresentava o seu \emph{lógos} soberano
  (\emph{lógos dynástes mégas estín,} Gor. \emph{Hel}. §8). Um dos
  ...
  ambos os trechos, a utilidade funciona como pseudofármaco, ou seja,
...
(\emph{asteîoi})\endnote{O termo \emph{asteîoi}, derivado de
  ...
  com homens da cidade.} e úteis ao povo (\emph{dêmôpheleĩs}).\endnote{O
...
mesmo trajeto, nem mesmo assim eu te abandonaria.\endnote{Heródico de
...
tempo e com empenho (\emph{en pollôi chr\gr{ό}nôi katà scholḕn}), \bekker{[228a]}
...
ouro.\endnote{Esse é um traço marcante da descrição da personalidade de
...
mesmo\endnote{Esse trecho será imitado por Fedro em 236c4-6.}, mas não
...
longo.\endnote{Assim como o próprio Fedro acabara de dizer ser incapaz
...
discursos (\emph{lógon}), ficas enternecido\endnote{Esse trecho será
...
lógon autón}). \bekker{[228e]} Se é assim, entendas uma coisa a meu respeito,
que eu não me prestaria ao teu exercício estando junto de Lísias
(\emph{paróntos dè kaì Lysíou})\endnote{A escrita aqui corresponde à
  ...
  da palavra falada.}, logo eu que o estimo (\emph{philô})\endnote{No
...
contigo.\endnote{Essa é a primeira imagem da mudança de curso figurado
...
\bekker{[229a]} S: Desviemo"-nos daqui, seguindo a direção do Ilisso, para nos
sentarmos em algum lugar que te pareça tranquilo.\endnote{Verbalização
...
estás assim.\endnote{A expressão ``oportunamente'' (\emph{eis}
...
F: Vês aquele elevadíssimo plátano?\endnote{\emph{Platano orientalis} é
...
\bekker{[229b]} F: Uma sombra, uma brisa moderada (\emph{pneûma
métrion})\endnote{Essa ``brisa moderada'' (\emph{pneûma métrion}) do
...
Bóreas raptado (\emph{harpásai}) Orítia?\endnote{O rapto na cultura
  ...
  Apuleio \emph{Met}. \versal{IV},28"-\versal{VI},24).}
...
donzelas que brincam (\emph{paídzein}) nessas margens.\endnote{A
...
\bekker{[229c]} S: Não é aqui, mas dois ou três estádios\endnote{Unidade de
...
(\emph{peíthêi})\endnote{Aqui (229c4-230a7) um ambiente semântico
...
versão, de que ela foi (\emph{hērpásthē}) raptada de lá e não daqui\endnote{Segundo Acusilao (\versal{DK}, 9, \versal{B}, 35) o rapto de Orítia por
  Bóreas teria ocorrido na Acrópole, quando a distinta filha do rei
  Erecteu levava um cesto de oferenda à deusa Atena.}. Eu
...
\emph{emautón}), de acordo com a inscrição délfica.\endnote{A inscrição
  délfica ``conhece-te a ti mesmo'' explica um forte aspecto da
  religiosidade de Sócrates, como uma medida (\emph{métrion}) apolínea
  que ele próprio buscava.} Pareceria risível \bekker{[230a]}, ainda
...
orgulhosa (\emph{epitethymménon}) que Tifon (\emph{Tuphônos})\endnote{Gerado
...
\bekker{[230b]} F: A própria.

S: Por Hera, que bela pousada!\endnote{Encontramos aqui, em 230b-c, uma
...
mármore (\emph{korôn te kaì agalmátōn}).\endnote{Figuras de terracota e
  de mármore ofertadas às Ninfas e a Aqueloo.} \bekker{[230c]} Se desejas
...
F: Parece"-me muito extravagante (\emph{atopṓtat\gr{ό}s})\endnote{Sócrates
  ...
  de mim possa ser amigo do que está no meu interior'' (279b8-c2).}, ó admirável. Pelo
que dizes, sem nenhum artifício (\emph{atéchnôs})\endnote{Essa \emph{atechnía} ou \emph{ausência de
  ...
  harmonia musical.}, dá"-me a impressão de
seres guiado como um estrangeiro e não como um autóctone. \bekker{[230d]}
...
somente os homens da cidade.\endnote{Essa posição citadina de Sócrates
...
(\emph{hēurēkénai}) o fármaco do meu êxodo.\endnote{Uma vez
  ...
  (\emph{Rep}. \versal{VII} 514b).} Tal como os que agitam um
...
\bekker{[230e]} Agora, tendo chegado aqui, vou reclinar"-me e encontrarás a
...
que pode acontecer conosco\endnote{Nessa recitação do discurso de
  ...
  extrair o máximo de prazer e evitar ao máximo os sofrimentos.}.~\bekker{[231a]} Espero que não me advenha nenhum
infortúnio (\emph{atychêsai}) só porque me ocorreu (\emph{tygchánō}) de
não estar te amando (\emph{ouk}~\emph{erastḕs})\endnote{A palavra \emph{erastès},
  ...
  contra a tradição.}, como aqueles que, tão logo tenha cessado o seu desejo
 (\emph{epithymías paúsōntai})\endnote{Expressão retomada em
  232e, 234a e em 264a, momento em que o início desse discurso será
  citado.}, arrependem"-se (\emph{metamélei}) do que bem fizeram. Outros, por outra
parte, não têm tempo hábil para mudar o pensamento (\emph{metagnônai}),
...
sofrimentos (\emph{pónon}) causados, consideram \bekker{[231b]} antiquadas as
...
consideram poder agradar (\emph{chareîsthai}) o companheiro. \bekker{[231c]}
...
\bekker{[231d]} De algum modo, é verossímil (\emph{eikós esti})\endnote{Esse
  ...
  retórica e aos seus fundamentos.}~aceitar
semelhante dificuldade àquele que passou por esse infortúnio, afinal,
quem se livraria (\emph{apotrépein})\endnote{O termo \emph{apotrepein} (231d1) ``livrar,
  ...
  verbais sem desconsiderar a proximidade conceitual.}~dele, mesmo sendo experiente
...
(\emph{phronoûsin}) mal, mas não podem dominar"-se (\emph{krateîn})\endnote{Aqui vemos o retrato inicial de Eros
  ...
  pensamento (\emph{eû} \emph{phronésantes}).}.~Como
...
Se receares a lei (\emph{nómon}) estabelecida\endnote{O termo
  ...
  educacional cf. Dover, \versal{K. J.} \emph{A Homossexualidade na Grecia
  Antiga} (1994), Marrou, \versal{H-I.} \emph{Histoire de l'Education dans
  L'Antiquite} (1948) e Pradeau \versal{J-F.} e Brisson, \versal{L.} \emph{Dictionnaire
  Platon} (2007), bem como Estrabão, \emph{Geografia}, \versal{X}, 4, 21 acerca
  dos costumes amorosos (\emph{perì tous érotas nómimon}) cretenses.}, não te afetes pelas
observações vergonhosas feitas pelos homens, pois é verossímil
(\emph{eikós esti}) \bekker{[232a]} que os amantes, considerando"-se honrados
...
o efeito do amor (\emph{mḕ}~\emph{erôntas})\endnote{Optamos pela paráfrase ``aqueles não estão sob
  ...
  estar sob o efeito de Eros.}~são superiores
(\emph{kreíttous}) ao escolher o melhor (\emph{béltiston}) em vez da
opinião (\emph{anti tês dóxês}) dos homens\endnote{Nesse discurso de Lísias ``o melhor'' é
  ...
  proferidos.}.~Ademais, necessariamente,
todos observam os amantes (\emph{erôntas}) com seus amados
(\emph{erôménois}), bem como as ações que praticam, e quando os veem a
conversar \bekker{[232b]} entre si, pensam que já consumaram ou que estão
para consumar o seu desejo (\emph{epithymías})\endnote{É possível perceber um predomínio, a
...
\bekker{[232c]} quando tudo aquilo que fazes de grandioso torna"-se
...
outro que possa deter esses bens. \bekker{[232d]} Quando persuadem"-no a
...
étychon}), mas pela virtude (\emph{aretḕn})\endnote{A prudência será
  identificada à virtude, que nesse contexto será evitar escolher alguém
  tomado pelo Amor.}~praticam o desejado, não
...
(\emph{epithymías paúsôntai})\endnote{Período idêntico em 231a. Nesse
  discurso há uma repetição da tópica do ``pausar o desejo'', evitar o
  impulso amoroso para evitar seus efeitos colaterais.}.~\bekker{[233a]} Para os que não estão amando
...
mim do que por um amante (\emph{erastêi})\endnote{Retomada da tese central desse discurso. O
  suposto autor do discurso, Lísias, tenta sobrepujar os erastas ou
  amantes cidadãos.}.~Os amantes elogiam as ações e
...
prazeroso (\emph{mḕ~hedonês})\endnote{O trecho é desenhado através de uma coordenação
  ...
  atividade, a elogiar aquilo de que não necessariamente gostavam. Talvez
  ...
  bastante comprometidos com a ``preocupação'' filosófica.}.~Assim, é muito mais conveniente
...
(\emph{chárin}), desejar"-nos-ão boas coisas\endnote{Nesse ponto o
  ...
  da expectativa da recompensa.}.~Da mesma maneira, convém
...
pauómenoi tês~epithymías})\endnote{Encontramos novamente a expressão ``cessar do
  desejo'' (\emph{pauómenoi tês epithymías}) como uma marca desse
  discurso de Lísias (Cf. 231a, 232e e 234a).}, buscarão pretexto para o ódio, enquanto os
...
ocorra o proveito (\emph{ôphéleían}) a ambos\endnote{Essa seria a síntese do discurso,
  ...
  será mais uma vez evocada por Theuth.}.~Eu considero o que foi
dito suficiente. Se desejares saber algo que foi negligenciado, pergunta
(\emph{erṓta})''\endnote{A última palavra do discurso, \emph{eróta},
...
(\emph{sunebákcheusa})\endnote{Sócrates simula um estado de afecção,
...
outra e, em ambos os casos, da melhor maneira possível.\endnote{Esse
...
pleíonos áxia}).\endnote{Expressão idêntica à de 236b42.}

S: Nesse ponto, eu jamais poderia ser persuadido (\emph{pithésthai}) por
ti. Os antigos sábios\endnote{Nesse uso do termo \emph{sophós}, Platão
  ...
  sábio novamente, logo à frente, em 235c3.}~(\emph{sophoì}), homens e mulheres que proferiram
...
encheram (\emph{peplêrôsthai}) como a uma vasilha\endnote{Esse
  ...
  quem fala, posto que é impulsionada por força alheia.}.~\bekker{[235d]} E foi por
...
(\emph{hypischnoûmai})\endnote{A promessa (\emph{hypischnéomai}) é o
  ...
  para Sócrates falar será um juramento (\emph{hórkos}) em 236d9.}, tal qual os nove arcontes, oferecer"-te um ícone
(\emph{eikóna})\endnote{O
  ...
  \emph{\versal{PMG}} frg. 192-193), ao mesmo tempo que remete ao juramento dos
  nove arcontes atenienses de não transgredir as leis, sob a pena de
  oferecer uma estátua em ouro (cf. [Arist.] \emph{Ath. Pol.} 7.1).}~dourado em tamanho natural (\emph{isométrêton}) no
templo de Delfos, não só o meu, mas também o teu.

\bekker{[235e]} S: És amicíssimo e de ouro verdadeiro, ó Fedro, se julgas que
...
necessariamente, terá ainda ele algo a dizer?\endnote{Aqui Sócrates
  ...
  Eros.}~Mas penso que é necessário
...
(\emph{heúresin})\endnote{São aspectos
  ...
  (invención, hallar, invención, p.111). Howland, \versal{R.L.}, associa a
...
mḕ~erôntos mâllon noseîn}), \bekker{[236b]} deixando o resto de lado, ao
dizer outro [discurso] mais extenso e mais digno (\emph{pleíô kaì
pleíonos áxia})\endnote{Expressão idêntica à de 235b4-5.}~que o de Lísias, estarás em ouro maciço junto à oferenda
dos Cipsélidas\endnote{Cípselo
  (657-627~a.C.), tirano de Corinto, pai de Periandro, um dos sete
...
sabedoria (\emph{sophían})\endnote{O termo sabedoria (\emph{sophían}),
  ligado a Lísias, mais uma vez é usado de modo irônico.}, outro [discurso] em algo mais variado.
...
estaria esquecido de mim mesmo''\endnote{Fedro imita a fala de Sócrates
  em 228a5-6.}, ou aquele ``desejas dizer, mas, no
entanto, fica enternecido''\endnote{Menção não literal de 228c1-2, quando Sócrates
  disse: ``ficas enternecido (\emph{ethrýpteto}) como se não desejasses
  falar''.}.~Põe na tua cabeça que não sairemos daqui
...
``vê se entende o que te digo''\endnote{Verso de Píndaro (frag. 105.1 Snell-Maehler) repetido
  no \emph{Mênon} 76d e em Aristófanes, \emph{Aves} v. 945.}, não queiras falar à força (\emph{pròs
...
algum deus daqui? -- \bekker{[236e]} e até mesmo a esse plátano\endnote{Esse
  juramento diante do plátano soa com tópica médica, da enunciação de um
  universo hipocrático ou, por contiguidade, apolíneo.}.~Se não
...
S: Que infâmia, ó abominável, bem encontraste (\emph{eũ anēũres})\endnote{Continuidade do uso do campo semântico de
  \emph{heurisko}: ``inventar'', ``encontrar'', ``descobrir''. Cf. nota
  63.}~um meio para forçar um homem que ama discursos
(\emph{philológôi})\endnote{Sócrates é amante desse \emph{lógos}, de
  todo tipo de discurso, falado e escrito, bem como obviamente do
  pensamento que ele porta.}~a fazer o que dizes.

F: Então porque te esquivas (\emph{stréphēi})?\endnote{O termo
...
\bekker{[237a]} F: Diz então.

S: Sabes como farei?

F: A respeito de quê?

S: Falarei encoberto (\emph{enkalypsámenos})\endnote{Cobrir a cabeça,
...
S: Vinde, ó Musas\endnote{Platão inicia a frase mimetizando o bordão de
  chamamento às Musas, inspiradoras da poesia. Cf. doxografia de
  Estesícoro (Eusthathius \emph{Sobre a} \emph{Ilíada} frag. 240,
  Campbell, \emph{Greek Lyric} \versal{III}, p.166), onde vemos: \emph{deur'áge,
  Kalliópeia lígeia}. Alcman também utiliza similar fórmula dedicada à
  Calíope: \emph{Mus'age Kalliópa thýgater Dìos} (fr. 84 \emph{\versal{PMG}},
...
momento, tenha agora melhor reputação (\emph{dóxêi}) ainda. \bekker{[237b]}
...
mḕ~erônti prò toû erôntos déoi charídzesthai})\endnote{Retomada da tese
...
dano (\emph{blabên})\endnote{Essa segunda introdução marca a
...
denominamos desmesura (\emph{hýbris})\endnote{Sócrates divide a ação humana em
  ...
  na palinódia (243e-257b6).}.~\bekker{[238a]} A desmesura
...
possua. \bekker{[238b]} Sobre a tirania (\emph{tyranneúsasa}) das bebidas,
...
correlatos, sempre são soberanos (\emph{dynasteuoúses}) e têm cada qual
o seu nome.
...
(\emph{hedonèn achtheîsa kállous}), \bekker{[238c]} então, os desejos
...
chamados de amor.''\endnote{Esse amor aquivale ao puro desejo dos
  ...
  necessário ressaltar, essa inspiração forjada e forçada que
...
[agrada] ao não afetado pelo amor\endnote{Essa retomada do
  ...
  está diretamente ligada ao vitupério e ao elogio.}.~\bekker{[238e]} Aos que começam a ser
...
(\emph{phronimṓtatos})\endnote{Nesse trecho o desejo é o grande mal a
...
philosophía})\endnote{A filosofia nessa perspectiva proporcionaria uma
  ...
  melhor (\emph{arístên}) das loucuras em 265b5.}.~Outras coisas são maquinadas para que [o amado] possa
...
(\emph{érôta})\endnote{Reafirmação da tese ímpia de Lísias contra o
  amor.}.
...
de prazer requintado (\emph{hêdonén tina ouk ámouson})\endnote{O
  ...
  (\emph{theríou poikílou kaì polykephálou}).}.~Como os
...
ditado, ``cada idade agrada aos da mesma idade''\endnote{O mesmo provérbio encontra-se
...
inteligência (\emph{noûn})\endnote{A dicotomia entre \emph{noûn} e
  \emph{anoétou} (241a8"-b1) que aparece nesse trecho é similar à
  dicotomia de \emph{anoétôi} e \emph{noûn}, logo a seguir (241b7"-c1),
  bem como entre \emph{noûs} e \emph{anoías} em 270a5.}~e salvo pela prudência
...
o intelecto (\emph{noûn})\endnote{Eros aqui aparecerá pela última vez
  como destruidor do intelecto.}.~Caso contrário, seria obrigado a entregar"-se
...
os cordeiros''\endnote{Segundo Hermias (§90.9-10 Couvreur = 61,12-13
  Lucarini e Moreschini) esse provérbio retoma \emph{Ilíada} \versal{XXII}, v.
  263.}.
...
que te ofereci sofra (\emph{páschein}) o que for\endnote{Suspeita, já
  ...
  tal narrativa ímpia.}.~\bekker{[242a]} Quanto a
...
de arte (\emph{atechnôs})\endnote{A palavra \emph{atechnôs} pode ser
  ...
  Fedro, na visão de Sócrates, não detém nenhuma arte.}.~\bekker{[242b]} Por conta dos discursos que
...
Símias de Tebas\endnote{Cf. \emph{Fédon} 59c.}, que é o mais forte (\emph{krateîs}) entre todos. E~agora tu me parece ser a causa (\emph{aitiós}) mesma de outro discurso
...
divino (\emph{hēmartēk\gr{ό}ta eis to theîon})\endnote{Acerca desse sinal
  costumeiro do \emph{daímon} de Sócrates cf. \emph{Euthyd}. 273e2-3,
  \emph{Theag}. 129b8, \emph{Pol}. \versal{VI} 496c3, \emph{Apol}. 31d1, 40a4,
  \emph{Alc}. \versal{I} 103a5, \emph{Theaet}. 151a4 entre outras passagens. Cf.
  outras ocorrências de \emph{daímon} no \emph{Lexicon Platonicum} de
  Friedrich Ast (1835) vol.1 p.415-417.}.~\bekker{[242c]} É que eu sou um
...
grámmata phaûloi}), no entanto para mim isso já é suficiente\endnote{Quando
  Sócrates diz que é um adivinho não muito aplicado, tal qual os que não
  são muito bons na escrita, parece visar Lísias ou Isócrates, que
  tinham reputação de bons escritores (logógrafos).}.~Aí então
...
que ela não altere minha honra (\emph{timàn}) junto aos homens\endnote{Cf. Campbell frg. 310, verso de Íbico também legado por
...
\bekker{[242d]} Só agora percebi a minha falta (\emph{ḗisthēmai tò
hamártēma}).
...
asebê}).\endnote{A impiedade aqui consiste em não perceber a ligação
...
disso, pretenderam"-se bondosos e muito civilizados (\emph{asteía})\endnote{Cf. nota 19 acerca de \emph{asteía}.}, mas
...
faltas em mitologia, Homero não a conheceu, mas Estesícoro\endnote{A
  Estesícoro de Himera (séc. \versal{VII} a.C.) atribui-se a invenção do terceiro
  ...
  e (c) epodo. Esse novo desenho é conhecido como \emph{pattern} \versal{AAB}. O
  ...
  \emph{pattern} estesicórico \versal{AAB}. Cf. D'Alfonso \versal{F.} \emph{Stesicoro et
  la performance}, Roma: \versal{GEI}, 1994.} sim. Privado da visão pela
...
imediatamente recuperou a visão (\emph{anéblepsen})\endnote{A
  \emph{Palinódia} de Estesícoro, assim como esta \emph{palinódia} de
  Sócrates, são como peãs ou hinos purificadores e curativos.}.~Eu, então, agora me
...
ocorrido, por vergonha, com a cabeça velada (\emph{egkekalumménos})\endnote{Cf. nota 72, referente 237a4, momento
...
(\emph{epithymô potímôi lógôi oîon halmuràn akoḕn apoklúsasthai})\endnote{O \emph{lógos} tem o poder catártico e de
  ...
  aproximada ao amor dos marinheiros, como algo vulgar.}.~Diga
...
pelo amor (\emph{mè erônti})\endnote{Resgate e inversão da tese central da
  \emph{palinódia} de Sócrates. Agora Sócrates fará elogio pleno do
  amor. Esse é um dos temas que o \emph{pattern} \versal{AAB} estesicórico
...
de Eufemo, natural de Himera\endnote{Nesta passagem demarca-se o
...
lógos})\endnote{Retomada do primeiro verso de Estesícoro, em
...
elas pouco ou nada fizeram.\endnote{O outro lado do discurso aparece
...
emphrónôn})\endnote{Essa sobriedade está relacionada à arte que surge
  entre os homens em 244d3: \emph{sôphrosýnes}.}~que buscam o futuro pelos pássaros e por outros sinais
...
(\emph{ek theoû}) e a outra junto aos homens (\emph{par'anthópôn})\endnote{Cf. oposição
...
kataschoménoi})\endnote{A loucura isenta da mácula, dos
...
sóbrios (\emph{sôphronoûntos})\endnote{Conservei nesta aparição de
...
\bekker{[245b]} Tenho dito a ti acerca da grandeza das belas obras repletas
...
(\emph{apodeiktéon aû tounantíon})\endnote{É necessário admitir o
  ...
  \emph{apódeixis} (cf. \emph{A.po} 71a-b).}: como pela maior das sortes essa
loucura é dádiva dos deuses. \bekker{[245c]} Essa demonstração
(\emph{apódeixis}) não será persuasiva (\emph{ápistos}) aos terríveis
(\emph{deinoîs}), mas será persuasiva (\emph{pistḗ}) aos sábios
(\emph{sophoîs})\endnote{Aqui \emph{sophoí} é usado sem ironia, ao
  ...
  engenhosidade e sofisticação da explicação racional dos mitos.}.~Será necessário primeiramente tratarmos da natureza da
...
\bekker{[246a]} Acerca da imortalidade é o suficiente. Falemos [agora]
acerca dessa ideia (\emph{idéas})\endnote{Forma, imagem, conceito
  ...
  ofuscamento e dos limites da própria linguagem.}.~Quanto ao que ela é, teríamos uma
...
e imortal (\emph{thnetón te kaì athánaton})\endnote{Cf. Heráclito, \versal{DK},
  12, \versal{B} 62: \emph{athánatoi thnêtoí, thnêtoì athánatoi} {[}\ldots{}{]}.} .~Toda alma ocupa"-se
...
dioikeî})\endnote{Escolhi para traduzir \emph{dioikeî} por ``habita'',
  ...
  \emph{controls} (De Vries, 1969, p.128).}, mas quando é levada à perda das asas, então, de algum sólido
...
sido expostas ao agrado do deus\endnote{A imagem é plasmada (\emph{pláttomen}) da
...
atribuída\endnote{\emph{tetagménoi, etáchthe} 247a2-3, sucessão de
  formas do verbo \emph{tásso}, ``ordenar'', ``dispor'', ``colocar em
  ordem'', bem como do substantivo correspondente \emph{táxis}.}.~Então, muitas divindades bem"-aventuradas seguem trajetos no
...
deuses (\emph{éxô theíou choroû})\endnote{Coro dos deuses, sendo que o verbo
  ...
  especialmente do movimento dos astros.}.~Quando vão ao cume para um festim ou
...
que a alma se dispõe\endnote{Cf. Plotino, \emph{Acerca do Belo} 7.31.}; as almas dos imortais, quando chegam ao extremo,
...
pálin}) para o interior do céu e chegam à casa\endnote{Héstia, única
  ...
  unidas aos corpos após a contemplação do ser.}. O
auriga chega ao estábulo, coloca ali os cavalos, oferece"-lhes ambrosia e
lhes dá néctar para beber.

\bekker{[248a]} Esta é a vida dos deuses. Quanto às outras almas, a que
melhor acompanha e se assimila (\emph{epoménê kaì}~\emph{eikasménê}) ao
deus\endnote{A assimilação entre homens e deuses aparece também em
  \emph{Teeteto} 176b: \emph{phygè dè homoíosis theôi kata to dynaton.
  homoíosis dè díkaion kai hósion metà phronéseos}.}, eleva a cabeça do auriga até o lugar exterior (\emph{tòn éxô
...
servem"-se do alimento da opinião (\emph{trophêi doxastêi chrôntai})\endnote{Como de costume na
...
ou aquele que fere o povo (\emph{demokopikós})\endnote{\emph{Kóptô}:
...
(\emph{moíras}), os que o fazem injustamente, o pior\endnote{Essa
  ...
  nessa escala.}.~Cada uma das almas
...
amante do saber (\emph{philosophḗsantos}) ou \bekker{[249a]} amante dos
jovens de acordo com a filosofia (\emph{paiderastḗsantos metà
philosophías})\endnote{Esse
  ...
  tutor ateniense.}. Estas, na terceira volta de mil anos, se
conduziram este tipo de vida por três vezes seguidas, no terceiro
milênio se afastam de modo alado. Quanto às outras, há as em que, ao término
...
justiça e são levadas à dignidade da vida humana que tiveram. \bekker{[249b]}
...
(\emph{airoûntai}) a que quiser\endnote{Com relação aos destinos das
  ...
  das almas.}.~Ali mesmo, os homens que foram feras
...
(\emph{anakúpsasa})\endnote{Cf. \emph{anakúptôn}, ``levantar a cabeça''
  ...
  109d-e).}~para o verdadeiro ser (\emph{tò ón óntôs}). É~por
...
valor servem"-se corretamente da recordação (\emph{hypomnḗmasin})\endnote{Servir-se das recordações
  ...
  (\emph{hypomnéma}), que são sempre externos.}, sempre
...
\bekker{[250b]} Justiça, prudência (\emph{dikaiosýnês mèn oûn kaì
sôphrosýnês}) e tantas outras preciosidades da alma não resplandecem
(\emph{phéggous})\endnote{Cf. Plotino, \emph{Acerca do Belo}, 4.9
...
revelação\endnote{Aqui a palavra revelação não refere o sentido comum
...
(\emph{kecharísthô}) pela memória (\emph{mnḗmêi}),\endnote{Essa dupla,
...
chegamos aqui \bekker{[250d]} tomando"-a com a máxima clareza de nossos
...
(\emph{phrónêsis}) não é vista\endnote{A prudência mostra a
...
Cairíamos em terríveis amores, se algum
...
\bekker{[250e]} Um recém"-iniciado ou alguém que veio a corromper"-se não é
...
\bekker{[251a]} O recém"-iniciado (\emph{artitelḗs}) que contemplou muitas
...
(\emph{hímeros})\endnote{Aqui \emph{hímeros} é o termo que designará
...
(\emph{pterophýtor})\endnote{A designação dos homens para o Amor,
...
encontram (\emph{heuróntes}) e tudo fazem para que permaneça assim\endnote{Reitera-se nessa passagem que o filósofo é a categoria
  ...
  maneira de referir Dion de Siracusa.}. Caso
...
esforço em olhar para o deus (\emph{pròs tòn theòn blépein})\endnote{Mais uma vez
  ...
  similitudes entre o deus e o amado, uma habilidade dialética.}.~\bekker{[253a]}
...
partilhar (\emph{matascheîn}) da divindade\endnote{A visão é uma
...
eleito é dessa maneira tomado\endnote{Aqui, depois de uma sucessão de
...
temperança e o pudor (\emph{sôphosýnês te kaì aidoûs})\endnote{Na
...
(\emph{táxin}) e o acordo (\emph{homologían})\endnote{Aqui há uma
  ...
  \emph{thymoeides} volte ao equilíbrio.}.~\bekker{[254d]} E novamente
...
\bekker{[254e]} O auriga sofre a maior dessas afecções (\emph{páthos
...
cessa sua desmesura (\emph{hýbreôs})\endnote{Cf. simetria em
...
vergonhoso associar"-se a um amante\endnote{Essa atitude de rechaço ao
  amante era a base da tese anteriormente defendida, mas agora essa tese
  aqui já aparece refutada.}, e que por isso tivesse repelido o
...
ninguém frente ao amigo entusiasmado (\emph{éntheon})\endnote{Hermias
...
acrescidas (\emph{anapterôsan}). \bekker{[255d]} Então, as vias das asas são
...
(\emph{ophthalmías})\endnote{A oftalmia aqui reaviva a tópica
  do poeta Estesícoro, curado da cegueira pela \emph{Palinódia}.} adquirida de outrem, ele não tem como expressar a
...
échôn})\endnote{\emph{Eidôlon}
  ...
  1,30,1, Leipzig: Teubner, 1967).}.~\bekker{[255e]} A este nomeia e considera não como amor
...
maioria (\emph{tôn pollôn}) toma por excelente e assim praticam\endnote{A multidão insensata é agora explicitamente
...
erastoû philía})\endnote{Ao contrário do que havia sido defendido, de
...
redor e debaixo da terra, num percurso sem intelecto (\emph{ánoun})\endnote{Aqui a velha tese de Lísias, repetida por
...
outras razões, mas especialmente no vocabulário\endnote{Retomada das
  palavras de Fedro em 234c.} poético a que fui
...
o seu próprio irmão Polemarco\endnote{Polemarco, irmão de Lísias,
  ...
  morte de seu irmão.}, para que este seu amante (\emph{erastès})
...
nós, que assim seja. \bekker{[257c]} O teu discurso há tempo que admiro
...
(\emph{ethelêsêi}) contra ele competir (\emph{antiparateînai})\endnote{Fedro reconhece somente aqui a
  superioridade de Sócrates frente a Lísias na composição de discursos.}. Pois é
...
escrever (\emph{gráphein})\endnote{Nesse ponto o diálogo ganha
...
(\emph{diamartáneis}), se o consideras como alguém tímido. \bekker{[257d]}
...
sofistas (\emph{mè sophistaì kalôntai})\endnote{Essa visão de que é
...
phronoûntes tôn politikôn})\endnote{Observa-se aqui uma menção aos
  ...
  censurados tanto em suas ações como nos seus discursos.}.~Em seguida, em discurso escrito
(\emph{gráphôsi lógon}), eles agradam aos seus panegiristas
(\emph{epainétas}), uma vez que estes são os primeiros a elogiá"-los
(\emph{epainôsin}), previamente e em qualquer situação\endnote{Os que elogiam e os elogiados nos discursos
  proferidos estabelecem uma conexão entre polos que alimentam
...
deus\endnote{Mais uma vez, a ideia de ser considerado um deus vem à
...
como poeta ou sem, como um prosador?\endnote{Esse exame
...
\bekker{[258e]} F: Perguntas (\emph{erôtais}) se precisamos? Que motivo teria
alguém para viver se não em vista, por assim dizer, desses mesmos
prazeres (\emph{hêdonôn})?\endnote{Pergunta similar à do início do
  ...
  sem beleza.} Pois não são daqueles que necessitam de
...
cigarras cantoras\endnote{Mais um lugar-comum (\emph{tópos}) da poesia
...
homens.\endnote{Aqui Platão recorre pela primeira vez no \emph{Fedro} a
  ...
  cigarras cantoras diriam deles se os vissem preguiçosos no discurso
...
realizadores\endnote{Hermias ao comentar o trecho, associa Calíope e
  ...
  §180.10-21 Couvreur).}. \bekker{[259d]} Érato (Amorosa) com a [poesia] erótica
...
homens\endnote{Deixei o nome grego praticamente
  ...
  v.~77-8 (Torrano, 1995, p.~109).}. Muitas são as razões para que falemos ao meio"-dia e não
cochilemos.
...
(\emph{eiduîan tò alêthès}) acerca do que será tratado?\endnote{Nesse
...
verdade (\emph{alêtheías})\endnote{Nesse parágrafo temos delineado o
...
S: ``Palavras nada desprezíveis''\endnote{Retomada da expressão da
...
alcançará a arte de persuadir (\emph{peíthein téchnei}).''\endnote{Agora
...
\bekker{[260e]} F: E ela não diria coisas justas ao proferir isso?
...
distintos tais quais Odisseu\endnote{Em Homero, Nestor é um lendário
...
adíkou})?\endnote{A partir daqui especialmente será possível observar o
...
pherómena})?\endnote{Eleático Palamedes é um epônimo de Zenão de Eleia.
...
\bekker{[261e]} S: Então, não só no tribunal e nas assembleias públicas
...
(\emph{apokruptôménou})\endnote{Depois de confirmar a aplicação da
...
predominantemente naquilo que difere muito ou pouco?
 ...
 \bekker{[262a]} F: No que difere pouco.
...
(\emph{tèn homoiótêta tôn óntôn kaì anomoiótêta akribôs dieidénai})\endnote{A arte de enganar, nesse retrato, tem êxito ao
...
de nenhuma arte no meu discurso (\emph{téchnês toû légein}).\endnote{Sócrates
  agora mimetiza a fala de Fedro, quando disse, em 229a3, que por
  ``sorte'' estava descalço, como Sócrates sempre estava. Conhecer a
...
\redondo{[…]}''.\endnote{Citação de 230e-231a, início da declamação de Lísias. Aqui é a
...
(\emph{planṓmetha})\endnote{Temas em que as pessoas podem facilmente
...
(\emph{to}~\emph{plêthos}~\emph{planâsthai}) e onde não\endnote{Uma definição provisória de arte
...
\bekker{[261c]} F: Belo seria, ó Sócrates, deter essa forma de apreensão
(\emph{katanenoêkṑs}).
...
arrependem"-se (\emph{metamélei}) do que bem fizeram.''\endnote{Citação
...
como digo, Fedro, querida cabeça\endnote{Cf. a mesma expressão
...
e anuncio aos que passam que Midas está aqui sepulto\endnote{Cf.
  Laércio, \versal{D.} \emph{A vida dos ilustres filósofos}, § 89.}.\\
\end{verse}
\end{quote}
...
divindade (\emph{hypò theías})\endnote{Cf. o discurso de Pausânias no
...
causa dos nossos maiores bens (\emph{agathôn})\endnote{Aqui vemos uma apresentação da dialética
  platônica, como arte intelectual de reunir e separar (cortar) ideias,
...
presenteá"-los como se fossem reis (\emph{basileûsin})? \endnote{Cf.
  Homero, \emph{Odisseia} \versal{VII}, v. 38. A menção a ``Trasímaco e outros
...
arte?\endnote{Aqui começa um ataque sistemático às concepções
...
de discursos (\emph{logodaídalon})\endnote{O epíteto de \emph{logodaídalon}, nesse
...
F: Mencionas o auspicioso Teodoro?\endnote{Em Aristóteles, Teodoro é
  considerado um inovador da linguagem, um criador de novidades
  (\emph{tà kainà}) (Arist. \emph{Rh}. \versal{III}, 11,6). O diagrama de
...
\bekker{[267a]} S: Quem mais senão ele? O qual disse haver nas composições
...
anḗr}).\endnote{Na \emph{Apologia} (Pl. \emph{Ap}. 20a-c), Sócrates
  ...
  que o exortava a compor música.} E Tísias\endnote{Juntamente com
  ...
  Τισίας ἐκαλεῖτο).} e Górgias\endnote{Capítulo à parte na história do
  ...
  por Platão, tratados exemplares da sua própria ``arte'' discursiva (\versal{DK}~82 p.~271"-307). 
  Cf. também o \emph{Górgias} de Platão, em que um
...
pequeno e o pequeno parecer grande, \bekker{[267b]} o novo parecer arcaico,
...
prolongamento indefinido?\endnote{Cf. sobre Górgias o estudo de
  Untersteiner \emph{I sofisti} (Milano: Mondadori, 1996, p.141-306), o
  de Mazzara, \versal{G.} (\emph{Gorgia, la retorica del verosimile}, Verlag,
  ...
  Aristóteles (\emph{Rh}. \versal{III}, 6).} Ouvindo isso de mim, outrora,
...
(\emph{metríôn}).\endnote{Pródico de Ceos (470/60-395 a.C.), famoso
  ...
  conquistada pela disciplina e pela moderação (cf. também \versal{DK} 84,
  p.308-319). Essa medida (\emph{metríon}) de que se gabava Pródico,
  encontrada já em Protágoras (\emph{métron} \versal{DK}, 80, \versal{B}1), é bastante
  presente na abordagem filosófica de Platão.}

F: Ó Pródico, sapientíssimo!

S: Não falamos ainda de Hípias?\endnote{Hípias de Élis (460-400 a.C.),
  ...
  a esse sofista (\emph{Hípias Maior} e \emph{Hípias Menor}). Cf. \versal{DK} 86,
  p.326-334.} Creio eu que o estrangeiro de Élis votaria também com
Pródico.

F: E por que não?

S: E o que diremos do \emph{Museu Discursivo} de Polo? \bekker{[267c]} Com
sua duplicação discursiva (\emph{diplasiologían})\emph{,} coleção de
máximas (\emph{gnômologían}) e estilo imagético
(\emph{eikonologian}).\endnote{Polo de Agrigento figura como discípulo
  ...
  \emph{Górgias} feita por Lopes, \versal{D. R. N.}, bem como a introdução que
  faz ao efeito cômico que Polo cumpre no diálogo, o qual chama de 2º
  Ato (\emph{Górgias}, trad. Lopes, \versal{D.}, São Paulo: Perspectiva, 2011,
  p.41"-79).} E do \emph{Vocabulário} que Licímnio\endnote{Licímnio de
  Quios é citado em Aristóteles (Arist. \emph{Rh}. \versal{III}, 1405b13; 1413b2
...
F: E de Protágoras\endnote{Protágoras de Abdera (492/0-422 a.C.), um
  dos mais ilustres sofistas, tem um pensamento de complexa reconstrução
  através da doxografía (\versal{DK}~80, p. 253-271). Autor da célebre frase ``o
  ...
  assuntos existem dois argumentos que se contrapõem (\versal{D.L.} 9,51), além
  ...
  (\emph{phainómenon eikós}) é usada na retórica e na erística (\versal{A.}
...
é o do grande Calcedônio\endnote{Trasímaco da Calcedônia (459-400 a.
...
kêleîn}), a dissipar essa fúria, como ele mesmo dizia. \bekker{[267d]}
...
perguntas''.\endnote{Nesse trecho em que Sócrates propõe esses diálogos
  imaginários, fica claro que pretende mostrar que os pseudoartífices,
...
\bekker{[268c]} F: Poderiam dizer, creio eu, que esse homem estivesse louco,
pois só por ter colhido de algum livro\endnote{Ao expor a pseudoarte,
...
holôi}) da composição\endnote{Nesse segundo diálogo imaginário, cujos
...
\bekker{[269a]} S: Então, Sófocles e Eurípides mostrariam que aquilo era
...
medicina mesma (\emph{tà iatriká})\endnote{Esse último diálogo
...
discursos breves (\emph{brachylogiôn}), dos estilos imagéticos
(\emph{eikonologiôn}) e tantos outros a que chegamos, os quais já
mencionamos e foram verificados à luz do dia? \bekker{[269b]} Qual deles
...
detentores de conhecimentos prévios necessários à arte. \bekker{[269c]} Por
...
discursos''. \endnote{Cf. em 267c1 \emph{eikonologían} ou ``estilo
...
imperfeito.\endnote{Esses três aspectos necessários ao orador,
  ...
  Hermogenis librum} perì staseôn, vol. 2, Ed. Rabe, \versal{H.} Leipzig:
  Teubner, 1893, p.4). O mesmo trecho é citado no \emph{Pròs Platona
  perì rethorikês} de Élio Aristides (Ed. Dindorf, \versal{W.} Leipzig: Reimer,
...
verossímil (\emph{eikótôs}) o maior perito na arte retórica\endnote{Sutil
...
anoías}),\endnote{Alguns tradutores preferem a transcrição de Burnet
  para o trecho (\emph{noû te kaì dianoías}), proveniente do códice \versal{V}
  (Vindobonensis 109), que determinaria a tradução por ``intelecto e
  pensamento'' ou algo similar. Na verdade, os códices \versal{B} (Bodleianus,
  \versal{MS. E.D.} Clarke~39) e \versal{T} (Venetus Append. Class.~4, cod.~1) trazem um
...
tôn lógôn téchnên tò prósphoron autêi}).\endnote{Essa tagarelice,
  ...
  vol. 2, Ed. Schenkl, \versal{H.}, Downey, \versal{G.}, Norman, \versal{A. F.} Leipzig: Teubner,
  1971, p. 329 c7).}
...
S: \bekker{[270b]} É que o recurso técnico (\emph{trópos téchnês}) da
medicina é similar ao da retórica\endnote{Fedro não entende bem o que
...
arte (\emph{mḕ~tribêi mónon kai empeiríai allà téchnêi})\endnote{Cf.
...
pode produzir ou ser afetada por algo externo? \endnote{A expressão
  \emph{alêthès lógos} foi estudada especialmente por Fattal, \versal{M.}
  ...
  acerca desse ponto cf. Cairus, \versal{H.} \emph{Da natureza do homem Corpus
  hippocraticum} (1999, p. 395-430) e Cambiano \versal{G.} \emph{Dialletica,
...
\bekker{[270e]} S: Sem esse método pareceríamos fazer uma travessia de cego
(\emph{typhloû poreíai})\endnote{Mais uma vez, a oftalmia de
  ...
  encanto que move, que libera de um miasma, que afasta da doença.}, mas não devemos comparar um cego ou um surdo
...
\bekker{[271a]} S: Então, o combate se estende por toda alma, pois a
...
inscreverão e produzirão uma visão da alma, em seguida observarão
...
\bekker{[271b]} S: Em terceiro lugar, é preciso realizar uma disposição entre
...
outros não são persuadidos\endnote{Há uma sutil ligação entre as
...
(\emph{brachylogias})\endnote{Cf. expressão similar,
...
discursos?''\endnote{Bloco textual importante por definir as
...
lembrar"-te e diz\endnote{Menção ao caminho curto pelo qual se poderia
...
(\emph{téchnên})\endnote{Sócrates expõe a definição clássica de
...
belíssimo\endnote{Mais uma vez Platão faz Sócrates conversar com um
...
F: Parece"-me que afirmas algo magnífico, ó Sócrates, se for assim mesmo.
...
jogo de dados e especialmente a escrita (\emph{grámmata}).\endnote{Hermias
...
deus.\endnote{No \emph{Político} 257b6, quando Sócrates conversa com
  ...
  oficial em Atenas (cf. Pausânias, \emph{Descrição} \versal{IX},16,1 e Píndaro,
  ...
  mantêm simplesmente o \emph{theòn}. Brisson (1989, p. 177 e 232),
...
vituperando.\endnote{Cf. na Nota 10 o sentido do termo ``utilidade'' e
...
(\emph{doxósophoi}) em vez de sábios (\emph{sophôn})''.\endnote{A
  alegoria sintetiza os elementos fundamentais do diálogo, como a
  diferença da memória (\emph{mnémes}) e da recordação
  (\emph{hypomnéseos}), no sentido de a escrita ser apenas uma memória
...
somente se é assim ou não\endnote{É importante observar a ligação
...
(\emph{gegramména})\endnote{O \emph{oráculo de Amon}, nesse caso,
...
poderoso?\endnote{Cf. especialmente em 265e-266b a divisão
...
(\emph{eídôlon}) seu\endnote{O \emph{eídôlon} é uma tópica
...
com relação às suas sementes?\endnote{Cf. \emph{Eutífron} 2d2-4
...
talêthḕs didáxai})\endnote{A expressão ``escrever na água escura''
...
``chegar''. E todos que buscam seguir seus passos\endnote{Cf.
...
\bekker{[276e]} F: Boa diversão frente àquela frívola, ó Sócrates, essa de
...
F: O belo Isócrates.\endnote{Hackforth, seguindo Wilamowitz, não
...
favorito, e tu, por sua parte, faça o mesmo ao teu Lísias\endnote{A equiparação de Isócrates como \emph{hetaîron} de Sócrates, de modo similar a Lísias com relação a Fedro, faz parecer, à primeira vista, que Sócrates (Platão) elogia Isócrates, especialmente porque diz adivinhar que sua natureza é superior à de Lísias, bem como que há algo de filosófico nele que poderá ainda vir à tona. Temos elementos suficientes para desconfiar desse elogio, pois Isócrates não era um novato, como Sócrates diz, e uma maneira de entender essa ideia é a comparação com Lísias, já que Isócrates parece ter ampliado sua atividade de logógrafo apenas quando voltou a Atenas durante o governo dos Trinta, período em que Lísias tinha já uma carreira consolidada -- apenas nesse sentido Isócrates era um ``novato''. Nessa temporalidade instaurada por Platão, Isócrates tem mais de 60 anos, ou seja, não parece que Sócrates acredite, nem Platão, que Isócrates chegará à filosofia, no sentido platônico do termo. Apesar de defender uma retórica que chamava de filosofia, apesar de atacar também os sofistas, Isócrates não tem uma concepção filosófica similar à de Sócrates/Platão. Além disso, na oratória, não poderia ser um modelo, pois não tinha boa dicção, e se reservava, assim como Lísias, ainda que por motivos diferentes, aos discursos escritos. É preciso compreender que Isócrates é um exemplo de ausência para Platão. Lísias não era ateniense, ou seja, escrevia para outros usarem seus discursos no tribunal, enquanto Isócrates, apesar de ser ateniense e poder participar das instâncias oficiais da cidade, nas quais naturalmente o discurso falado era importante, nunca foi capaz disso, primeiro porque esteve fora durante todo o período da guerra do Peloponeso, além da limitação performática de não falar em público e ficar restrito também aos discursos escritos, como modelos, exercícios didáticos e discursos sob encomenda. De alguma maneira Lísias e Isócrates se completam, em níveis diferentes, como contraexemplos daquilo que Platão defende como filosofia ou como retórica filosófica, nas quais se deve falar e escrever dentro dessa nova prescritiva, agora dialética, das artes discursivas.}.
...
\bekker{[279c]}\endnote{Sócrates nesse epílogo em forma de prece desfaz a
...
amigos.\endnote{Esse dito pitagórico é mencionado na \emph{República}
